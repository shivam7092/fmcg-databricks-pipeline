...
# MAGIC %md
# MAGIC ### Load Project Utilites & Initialize Notebook Widgets
...
# 1. Parse 'month' from multiple data formats

data_formats = ["yyyy/MM/dd","dd/MM/yyyy","yyyy-MM-dd","dd-MM-yyyy"]
...
# MAGIC %md
# MAGIC #### 2.Handling the gross price
...
# we are validating the gross_price column, converting only valid numeric values to double, fixing negetive prices by making them positive, and replacing all non-numeric values with 0
...
# we enrich the silver dataset by performing an inner join with the products table to fetch the correct product_code for the each product_id
...
# MAGIC %md
# MAGIC ### Mering data source with the parent 
...
# MAGIC %md
# MAGIC - #### Get the price for each product_code (aggregated by the user)
...
df_gold_latest_price = df_gold_latest_price.select("product_code","year","gross_price")\
    .withColumnRenamed("gross_price", "price_inr").select("product_code", "price_inr","year")

# changeing year to string
...
# merging it to gold parent table using upsert operation
